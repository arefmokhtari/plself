...
bot = TelegramClient(pl.Conf.SESSION_DIR + pl.Conf.SESSION_API_NAME, pl.Conf.API_ID, pl.Conf.API_HASH).start(
    bot_token=pl.Conf.BOT_TOKEN)
...
async def GetMsGServic3InGP(event: events.chataction.ChatAction.Event):
    chat_id, user_id = event.chat_id, getattr(event.action_message, 'from_id', None) and \
...
async def InsTA(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd == 3:
        if cmd[1] == 'post':
            insta = pl.instaBot(pl.Conf.INSTAGRAM[0], pl.Conf.INSTAGRAM[1], pl.Conf.INSTAGRAM[2],
                                pl.Conf.SESSION_DIR[:-1])
...
async def GeTCal(event: events.newmessage.NewMessage.Event):
    try:
        await pl.send_sudo_msg(event, f'`{eval(event.raw_text[4:]):,}`', Account)
    except:
        await pl.send_sudo_msg(event, f'**{pl.rand_ch()} error !**', Account)
...
async def FinDManageR(event: events.newmessage.NewMessage.Event):
    ...
    if event.is_reply:
        ...
        if msg.media and type(
                msg.media) is types.MessageMediaDocument and msg.media.document and msg.media.document.attributes:
            if len_cmd >= 2:
...
async def ReBaSE(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd >= 3 and cmd[1].isdigit():
        await pl.send_sudo_msg(event, pl.Base(event.raw_text[event.raw_text.find(' ', 5) + 1:], int(cmd[1])).result(),
                               Account)
...
async def ReMorsE(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd >= 2:
        await pl.send_sudo_msg(event,
                               ''.join([pl.switch(morse, pl.CoDMORsE, ' ') + ' ' for morse in event.raw_text[6:]]),
                               Account)
...
async def RuNCoD3(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd >= 2:
        ...
        if cmd == 'help':
            ...
            try:
                s = subprocess.run(['g++', '-std=c++17', file], capture_output=True, text=True, timeout=5)
            except subprocess.TimeoutExpired:
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} timeout error !**', Account)
            else:
                if s.stderr:
                    await pl.send_sudo_msg(event, f'**{pl.rand_ch()} error:**\n\n`' + s.stderr + '`', Account)
                else:
                    try:
                        code = subprocess.run(['./a.out'], capture_output=True, text=True, timeout=5)
                    except subprocess.TimeoutExpired:
                        await pl.send_sudo_msg(event, f'**{pl.rand_ch()} timeout error !**', Account)
                    else:
                        await pl.send_sudo_msg(event, f'**{pl.rand_ch()} result:**\n\n`' + code.stdout + '`', Account)
                    finally:
                        os.remove('a.out')
        elif cmd == 'program':
            try:
            ...
            pl.edit_source_run(event.raw_text[event.raw_text.find('\n') + 1:], file)
            try:
                s = subprocess.run(['gcc', file, '-o', 'a.out'], capture_output=True, text=True, timeout=5)
            except subprocess.TimeoutExpired:
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} timeout error !**', Account)
            else:
                if s.stderr:
            ...
            pl.edit_source_run(event.raw_text[event.raw_text.find('\n') + 1:], file)
            try:
                s = subprocess.run(['javac', file], capture_output=True, text=True, timeout=5)
            except subprocess.TimeoutExpired:
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} timeout error !**', Account)
            else:
                if s.stderr:
                    ...
                    try:
                        code = subprocess.run(['java', 'source'], capture_output=True, text=True, timeout=5)
                    except subprocess.TimeoutExpired:
                        await pl.send_sudo_msg(event, f'**{pl.rand_ch()} timeout error !**', Account)
                    else:
                        await pl.send_sudo_msg(event,
...
async def GetLyricZ(event: events.newmessage.NewMessage.Event):
    ...
    if len(text) == 6 and event.is_reply:
        msg = await event.get_reply_message()
        if msg.media and 'document' in msg.media.to_dict() and 'attributes' in msg.media.document.to_dict() and 'title' in \
                msg.media.document.attributes[0].to_dict():
...
async def SeYTime(event: events.newmessage.NewMessage.Event):
    ...
    await pl.send_sudo_msg(event, f'{pl.rand_ch()} Tim3 NoW :\n' + '- time = %.2d:%.2d:%.2d' % (
        ...
        Dat3[1]) + ' - ' + pl.send_seasons(Dat3[1], 'j') + '\n- Month = ' + pl.jdmonthname(
        Dat3[1]) + ' - ' + dt.now().strftime("%B"), Account)
...
async def TranslatE(event: events.newmessage.NewMessage.Event):  #
    ...
    if event.is_reply and len(event.raw_text.split()) == 2:
        ...
        msg_for_tr = re.search(r'^({pre_re}|)tr(\s*)([a-zA-Z]{2}) ((.|\s)*)$'.replace("{pre_re}", pre_re),
...
async def SeYInFO(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd == 1:
        ...
        await pl.send_sudo_msg(event, f'**{ch} user in pv:**\n\n' + '\n'.join(
            map(lambda s: f'{c.get_num()} - [{s}](tg://user?id={s})',
                clir.lrange(pl.DataBase.users_in_private, 0, -1))), Account)
...
async def SenDSaVOicE(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd >= 2:
        if cmd[1] == 'play' and event.is_reply:
            msg = await event.get_reply_message()
            if msg.media and getattr(msg.media.document, 'attributes', None) and type(
                    msg.media.document.attributes[0]) is types.DocumentAttributeAudio and msg.media.document.attributes[
                0].voice:
                file = await msg.download_media()
                try:
                    ...
                    await pl.send_sudo_msg(event, f'{txt}' or f'**{pl.rand_ch()} is empty !**', Account)
            ...
            if voice_name not in clir.hgetall(pl.DataBase.voices).keys():
                ...
                if msg.media and type(
                    ...
                    await pl.send_sudo_msg(event, f'**{pl.rand_ch()} done, voice name to call :** `{voice_name}`',
                                           Account)
            else:
            ...
            if voice_name in clir.hgetall(pl.DataBase.voices).keys():
                ...
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} done,** `{voice_name}` **removed to database !**',
                                       Account)
            else:
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} the** `{voice_name}` **not in database !**', Account)
        elif cmd[1] == 'list':
            num = pl.Counter()
            await pl.send_sudo_msg(event, f'{pl.rand_ch()} **voice list:**\n\n' + '\n'.join(
                map(lambda x: f'**{num.get_num()} -** `{x}`', clir.hgetall(pl.DataBase.voices).keys())), Account)
        else:
            voice_name = cmd[1]
            if voice_name in clir.hgetall(pl.DataBase.voices).keys():
    ...
    if len_cmd >= 2:
        if cmd[1] == 'save' and event.is_reply and len_cmd == 3:
            ...
            if file_name in clir.hgetall(pl.DataBase.files).keys():
                ...
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} done,** `{file_name}` **removed the database !**',
                                       Account)
            else:
            ...
            await pl.send_sudo_msg(event, f'**{pl.rand_ch()} file list:**\n\n' + '\n'.join(
                map(lambda x: f'**{num.get_num()} -** `{x}`', clir.hgetall(pl.DataBase.files).keys())), Account)
        else:
            file_name = cmd[1]
            if file_name in clir.hgetall(pl.DataBase.files).keys():
                reply_to = ''
...
async def LockGpManager(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd > 1:
        ...
        if database:
            ...
            if cmd[1] == 'settings':
                await pl.send_sudo_msg(event, '\n'.join(
                    map(lambda k: f'**{ch}** `{k[0]}` **=> [{"✔️" if k[1] else "✖️"}]**', database.items())), Account)
            elif cmd[0] == 'lock':
    ...
    if event.is_group:
        if text == 'mute all':
            if chat_id not in clir.lrange(pl.DataBase.mute_all, 0, -1):
                ...
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} group** `{event.chat_id}` **has been muted !**',
                                       Account)
            else:
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} group** `{event.chat_id}` **has been muted before !**',
                                       Account)
        elif cmd[0] == 'mute':
            user = None
                ...
                if int(user) not in sudo:
                    if chat_id in clir.hgetall(pl.DataBase.mute_group_user).keys():
                        if user not in clir.hget(pl.DataBase.mute_group_user, chat_id).split():
                            ...
                            await pl.send_sudo_msg(event, f'**{pl.rand_ch()} user** `{user}` **has been muted !**',
                                                   Account)
                        else:
                            await pl.send_sudo_msg(event,
                                                   f'**{pl.rand_ch()} user** `{user}` **has been muted before !**',
                                                   Account)
                    else:
                        pl.adduserinMuteGp2hset(clir, pl.DataBase.mute_group_user, chat_id, user)
    ...
    if event.is_group:
        if text == 'unmute all':
            if chat_id in clir.lrange(pl.DataBase.mute_all, 0, -1):
                ...
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} group** `{event.chat_id}` **it has out of muted !**',
                                       Account)
            else:
                await pl.send_sudo_msg(event, f'**{pl.rand_ch()} group** `{event.chat_id}` **has not been muted !**',
                                       Account)
        elif cmd[0] == 'unmute':
            user = None
            if len_cmd == 1 and event.is_reply:
                ...
                if int(user) not in sudo:
                    if chat_id in clir.hgetall(pl.DataBase.mute_group_user).keys():
                        if user in clir.hget(pl.DataBase.mute_group_user, str(event.chat_id)).split():
                            ...
                            await pl.send_sudo_msg(event, f'**{pl.rand_ch()} user** `{user}` **has been unmuted !**',
                                                   Account)
                        else:
                            await pl.send_sudo_msg(event, f'**{pl.rand_ch()} user** `{user}` **has no muted !**',
                                                   Account)
                    else:
            ...
            if cmd[1] == 'bio':
                ...
                if cmd[2] == 'on':
                    ...
                    if get_re:
                        ...
                        await pl.send_sudo_msg(event, f'**{pl.rand_ch()} lasttime was already OFF !**', Account)
            elif cmd[1] == 'biotime':
                if cmd[2] == 'on':
...
async def SpeeDTesT(event: events.newmessage.NewMessage.Event):
    ...
    if len_cmd == 1:
        ...
        await msg.edit(
            f'**{ch} result from `speedtest.net` after {(dt.now() - FuckingTIME).seconds}s :**\n\n**{ch} download :** `{res["download"]:,}` **Mbps**\n**{ch} upload :** `{res["ping"]:,}` **Mbps**\n**{ch} ping :** `{res["ping"]}` **ms**')
...
async def TurNFuckinGOff(event: events.newmessage.NewMessage.Event):
    await event.edit(f'**{pl.rand_ch()} bot went offline !**') if event.sender_id in Account else await event.reply(
        f'**{pl.rand_ch()} bot went offline !**')
...
async def AddGrouP(event: events.newmessage.NewMessage.Event):
    if (str_chat_id := str(event.chat_id)) not in clir.hgetall(pl.DataBase.group_manager).keys():
        ...
        await pl.send_sudo_msg(event, f'**{pl.rand_ch()} group added to database before !**', Account)
...
async def RemGrouP(event: events.newmessage.NewMessage.Event):
    if (str_chat_id := str(event.chat_id)) in clir.hgetall(pl.DataBase.group_manager).keys():
        ...
        await pl.send_sudo_msg(event, f'**{pl.rand_ch()} group deleted to database before !**', Account)
...
async def ChTabchi(event: events.InlineQuery.Event):  # 'ChTabchi '+data+' '+str(event.sender_id)
    if event.query.user_id in Account:
        ...
        builder = event.builder
        result = await builder.photo(pic_name + '.png',
                                     text=f'با سلام [کاربر](tg://user?id={user}) گرامی:\nلطفا کد کپچای درست را انتخاب کنید.',
                                     buttons=buttons)
        await event.answer([result])
...
async def gpanel_1(event: events.InlineQuery.Event):
    if event.query.user_id in Account:
        ...
        builder = event.builder
        result = await builder.article(title='panel',
                                       text=f'{ch} Welcome to the management panel of Group!\nPlease choose: {ch[::-1]}',
                                       buttons=buttons)
        await event.answer([result])
...
async def main_call(event: events.CallbackQuery.Event):
    ...
    if (event.data.split()[0] == b"ftabchi" or event.data.split()[0] == b"ttabchi"):
        if event.original_update.user_id == int(event.data.split()[1]):
            ...
            await event.answer('برای تو نیستش.', alert=True)
        ...
        try:
            if event.data == b"exitpl":
                ...
                return await event.edit(
                    'Muted User in Pv :' + ' - '.join(clir.lrange(pl.DataBase.mute_private_user, 0, -1)),
                    buttons=buttons)
            elif event.data == b"list_mute_gp":
                return await event.edit('Muted User in GrouP :', clir.hgetall(pl.DataBase.mute_group_user),
                                        buttons=buttons)
...
scheduler.add_job(ChangeLasTName, "interval", minutes=1,
                  next_run_time=f'{dt.today().year}-{dt.today().month}-{dt.today().day} {dt.today().hour}:{dt.today().minute}:00')
scheduler.add_job(ChangeBioTimE, "interval", hours=1,
